...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.put(`/api/jobs/${route.params.id}`, updatedJob)
    ...
    <div class="container m-auto max-w-2xl py-24">
      <div class="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
        <form @submit.prevent="handleSubmit">
          ...
          <div>
            <button
              class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-full w-full focus:outline-none focus:shadow-outline flex items-center justify-center gap-2"
              type="submit"
              :disabled="isLoading"
            >
              <PulseLoader v-if="isLoading" color="#ffffff" size="8px" />
